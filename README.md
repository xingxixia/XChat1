...
当前版本是 XChat1 的第一版，主要目标不是重做一个全新的软件，而是在 ZcChat2 原有基础上完成一次清晰的二次开发：

- 保留原项目的 Qt6 Widgets 桌面端结构
- 保留原项目的角色立绘、对话、设置、语音合成和插件系统基础
- 将体验方向收束到“桌面 AI 角色陪伴”
- 修正使用过程中遇到的显示、输入、设置窗口和 API 配置问题
- 增加少量适合 Galgame 风格聊天框的文本设置能力

## 基于上一版的修改

这里的“上一版”指原项目 [ZcChat2](https://github.com/Zao-chen/ZcChat2)。XChat1 第一版在它的基础上主要做了这些修改：
...
第二版继续把项目方向收束到“一个固定桌宠个体”的体验上。

这一版重点不是增加复杂功能，而是把桌宠个体的提示词配置、历史对话维护和基础交互问题补齐，让后续测试 API、调试人设和维护上下文更方便。

## 基于第一版的修改

XChat1 第二版在第一版基础上主要做了这些修改：

- 修正聊天框隐藏后历史对话窗口仍停留在桌面上的问题
- 新增“桌宠行为设定”文本框，用于在设置界面维护桌宠基础提示词
- 将 AI 调用中的桌宠身份提示词从硬编码改为优先读取 `petPrompt`
- 隐藏角色选择、导入、导出、删除入口，进一步收束为单个桌宠个体
- 删除角色导入、导出、删除相关的 C++ 槽函数和压缩/解压辅助逻辑
- 在角色设置底部增加页码切换
- 新增历史对话维护页，支持刷新、清空、编辑保存、删除、删除一轮和回滚
- 设置页修改历史后会通知聊天框重新加载历史，避免内存历史和文件历史不一致
- 历史维护页用“你 / 她 / 记录”和淡色背景区分不同类型记录
- 修正历史维护页中文字和按钮宽度过窄导致的显示溢出问题

第二版仍然暂不实现长期记忆、自动总结和历史发送数量限制。这些会等记忆系统设计更清楚后再做。
...
程序运行时会优先读取项目目录下的运行配置：

```text
runtime_data/ZcChat2
```
